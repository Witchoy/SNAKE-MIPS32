...
.data
textEnd: .asciiz "Your score is of : \n"
sautligne: .asciiz "\n"
...
changeDir:
####On vérifie que l'on ne cherche pas aller dans la direction opposé, ce qui n'est pas possible####
lw $t1, snakeDir
sub $t1, $a0, $t1
li $t2, 2
li $t3, -2
...
NonMouvQueue:
#####Mouvement de la tête (qui prends en compte la direction choisis par le joueur) #####
lw $t0, snakeDir,
li $t1, 0,
beq $t0, $t1, AvanceH
li $t1, 1,
...
QueueCollisionLoop:
bge $t6, $t5, conditionFinJeuFin  # Fin de la boucle si on a parcouru toute la queue
lw $t7, snakePosX($t6)
lw $t8, snakePosY($t6)
...
conditionFinJeuFin: #Le jeu continue
li $v0 0
jr $ra

FinjeuON: #Le jeu se stop
li $v0 1
jr $ra
